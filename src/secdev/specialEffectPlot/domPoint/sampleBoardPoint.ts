...
export default class sampleBoardPoint extends domPointBase {
    ...
    #addDom() {
        ...
        const $pointDiv = this.$container.querySelector(".sample-label-point") as HTMLElement;
        $pointDiv.style.backgroundImage = `url(${require("@/secdev/assets/img/sampleLabelPoint/point.png")})`;
        $pointDiv.onclick = ()=>{
            this.#setBoardVisible(!this.#boardVisible);
        }
        const $lineDiv = this.$container.querySelector(".sample-label-board-line") as HTMLElement;
        $lineDiv.style.backgroundImage = `url(${require("@/secdev/assets/img/sampleLabelPoint/pedestal.png")})`;
