    ...
    <CommPanel title="坡度分析" class="slope-panel-box">
        <div class="slope-panel">
            <div class="slope-slider">
                ...
                <CommSlider v-model="steps" :min="10" :max="100" :step="1"></CommSlider>
            </div>
            <div class="btn-group">
                <CommButton @click="drawRect" class="comm-btn">绘制</CommButton>
                <CommButton @click="destroy" class="comm-btn" contentClass="clear">清空</CommButton>
            </div>
        </div>
    </CommPanel>
    <CommPanel title="坡度图例" class="slope-legend-box">
        <div class="legend-item-box">
            <div class="legend-item" v-for="(item, i) in legendList" :key="i">
                ...
                ></span>
                <span class="legend-label">{{ Number(item.min).toFixed(2) }}</span>
                <span style="color: #fff">-</span>
                <span class="legend-label">{{ Number(item.max).toFixed(2) }}</span>
            </div>
        </div>
    </CommPanel>
</template>
...
import slopeAnalysis, { countColorType } from "@/secdev/spatialAnalysis/slopeAnalysis";
